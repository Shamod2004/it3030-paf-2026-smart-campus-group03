              ...
              {notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <div key={n.id} style={s.dropItem}>
                    ...
                    <button style={s.markReadBtn} onClick={() => handleMarkRead(n.id)} title="Mark read">✓</button>
                  </div>
                ))
              )}
            </div>
...
const s = {
  ...
  markReadBtn:   { background: 'rgba(16,185,129,0.15)', border: 'none', color: '#10b981', width: 26, height: 26, borderRadius: 6, cursor: 'pointer', fontSize: 13, flexShrink: 0 },
