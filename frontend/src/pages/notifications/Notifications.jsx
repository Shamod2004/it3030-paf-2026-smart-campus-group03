...
const TYPE_META = {
  ...
  TICKET_CREATED:    { icon: '🆕', color: '#f59e0b', label: 'New Ticket' },
  TICKET_REJECTED:   { icon: '❌', color: '#ef4444', label: 'Ticket Rejected' },
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        <p style={{ ...s.message, fontWeight: n.isRead ? 400 : 600 }}>
          {n.senderName ? <span style={{ color: '#818cf8', fontWeight: 700 }}>{n.senderName}: </span> : ''}
          {n.message}
        </p>
        {n.referenceId && (
          <p style={s.ref}>
      ...
      <div style={s.rowActions}>
        {!n.isRead && (
          <button style={s.btnRead} onClick={onMarkRead}>
            Read
          </button>
        )}
        <button style={s.btnDel} onClick={onDelete}>
          Delete
        </button>
      </div>
    </div>
...
const s = {
  ...
  btnRead:    { background: 'rgba(16,185,129,0.15)', border: 'none', color: '#10b981', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 },
  btnDel:     { background: 'rgba(239,68,68,0.1)', border: 'none', color: '#ef4444', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 },
